...
```{r}
all_data_model = read_rds(here('results/single_model_expression_regression/final_model_500feat.rds'))
```
...
cross_val = MIBS_preds %>% ggplot(aes(x=imputed_viability,y=CV_pred)) +
	...
	ggtitle(paste0("R2: ", signif(cv_r2,3), "  RMSE: ", signif(cv_rmse,3))) +
	# xlim(c(0,2)) + ylim(c(0,2)) +
	BerginskiRMisc::theme_berginski()

MIBs_adj = MIBS_preds %>% ggplot(aes(x=imputed_viability,y=certain_adj_pred)) +
	geom_point(size=0,alpha=0.25) +
	geom_abline(slope = 1, intercept = 0,linetype=2) +
	geom_smooth(method = 'lm') +
	labs(x="Actual Imputed Viability",y="MIBs Adjusted Viability Prediction") +
	ggtitle(paste0("R2: ", signif(certain_adj_r2,3), "  RMSE: ", signif(certain_adj_rmse,3))) +
	# xlim(c(0,2)) + ylim(c(0,2)) +
	BerginskiRMisc::theme_berginski()
...
no_adj_plot = MIBS_preds %>% ggplot(aes(x=imputed_viability,y=no_adj_pred)) +
	...
	ggtitle(paste0("R2: ", signif(no_adj_r2,3), "  RMSE: ", signif(no_adj_rmse,3))) +
	# xlim(c(0,2)) + ylim(c(0,2)) +
	BerginskiRMisc::theme_berginski()

ggsave(here('figures/single_model_expression/MIBS_adj_vs_CV.png'), cross_val + MIBs_adj + no_adj_plot, width=8*(3/2),height=4)
