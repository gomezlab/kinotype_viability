...
dir.create(here('results/single_model_via_50'), recursive = T)
...
binarized_viability = PRISM_klaeger_CCLE %>%
	...
	write_rds(here('results/single_model_via_50/klaeger_CCLE_binary_full.rds'), compress = 'gz')
toc()
```

```{r}
dir.create(here('results/single_model_via_50/CV_splits/'))
...
for (i in 1:10) {
	...
	)	%>% write_rds(here('results/single_model_via_50/CV_splits/',sprintf('%02d.rds',i)), compress = 'gz')
}
```