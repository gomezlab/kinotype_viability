...
if (file.exists(here('results/single_model_expression_regression_LOLO/CV_split_row_nums.rds'))) {
	fold_ids = read_rds(here('results/single_model_expression_regression_LOLO/CV_split_row_nums.rds'))
} else {
	cell_line_id_nums = data.frame(depmap_id = unique(PRISM_klaeger_imputed$depmap_id)) %>%
		mutate(line_id = 1:n())
	
	fold_ids = PRISM_klaeger_imputed %>%
		left_join(cell_line_id_nums) %>% 
		pull(line_id)
	
	write_rds(fold_ids, here('results/single_model_expression_regression_LOLO/CV_split_row_nums.rds'))
}
