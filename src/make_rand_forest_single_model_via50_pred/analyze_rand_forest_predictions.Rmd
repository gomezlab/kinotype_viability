...
```{r}
prediction_results = read_rds(here('results/single_model_via_50/final_prediction_results.rds')) %>%
	ungroup()
...
dir.create(here('figures/single_model_via_50/avail_lines_predictions/'), recursive = T, showWarnings = F)

for (this_line in unique(avail_prediction_results$cell_line_name)) {
	this_plot = get_interesting_pred_plots(this_line)
	ggsave(here('figures/single_model_via_50/avail_lines_predictions/',paste0(this_line,".png")), this_plot, width=15,height=5)
}
```
...
ggplot(line_diff_pred, aes(x=log10(concentration_M), y=mean_pred,color=cell_line_name)) +
	...
	facet_wrap(~drug)
ggsave(here('figures/single_model_via_50/avail_lines_predictions/SK-BR_vs_SUM159.png'))
```
