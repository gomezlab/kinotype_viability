...
```{r}
dir.create(here('figures/feature_selection'))
...
ggsave(here('figures/feature_selection/feature_cor_histograms.png'),width=7)
BerginskiRMisc::trimImage(here('figures/feature_selection/feature_cor_histograms.png'))
```
...
ggplot(all_cor, aes(x=rank,y=sub_percent_rank,color=feature_type)) +
	...
	BerginskiRMisc::theme_berginski()
ggsave(here('figures/feature_selection/group_perc_rank.png'),width=6*0.75,height=5*0.75)
BerginskiRMisc::trimImage(here('figures/feature_selection/group_perc_rank.png'))
...
ggplot(all_cor %>% filter(rank <= 2000), aes(x=rank,y=sub_percent_rank,color=feature_type)) +
	...
	labs(x="Feature Type Overall Correlation Rank",y="Percentile Sub Rank", color='') +
	BerginskiRMisc::theme_berginski()
ggsave(here('figures/feature_selection/group_perc_rank_zoom.png'),width=6*0.75,height=5*0.75)
BerginskiRMisc::trimImage(here('figures/feature_selection/group_perc_rank_zoom.png'))
```
...
ggplot(all_cor, aes(x=rank,y=sub_rank,color=feature_type)) +
	...
	BerginskiRMisc::theme_berginski()
ggsave(here('figures/feature_selection/group_rank_count.png'),width=6*0.75,height=5*0.75)
BerginskiRMisc::trimImage(here('figures/feature_selection/group_rank_count.png'))
...
ggplot(all_cor %>% filter(rank <= 2000), aes(x=rank,y=sub_rank,color=feature_type)) +
	...
	labs(x="Feature Type Overall Correlation Rank",y="Number of Features", color='') +
	BerginskiRMisc::theme_berginski()
ggsave(here('figures/feature_selection/group_rank_count_zoom.png'),width=6*0.75,height=5*0.75)
BerginskiRMisc::trimImage(here('figures/feature_selection/group_rank_count_zoom.png'))
```
...
```{r}
if (file.exists(here('results/single_model_all_data_via50/CV_split_row_nums.rds'))) {
	fold_ids = read_rds(here('results/single_model_all_data_via50/CV_split_row_nums.rds'))
} else {
	fold_ids = sample(rep(1:10,length.out = dim(PRISM_klaeger_imputed)[1]))
	write_rds(fold_ids, here('results/single_model_all_data_via50/CV_split_row_nums.rds'))
}
...
rand_forest_grid <- grid_regular(
	...
	finalize(mtry(),),
	levels = c(3,3,2)
) %>% write_rds(here('results/single_model_all_data_via50/hyper_param_search_space.rds'))
```
...
for (i in 1:10) {
	...
	for (feature_num in c(200,500,1000,1500)) {
		target_dir = here('results/single_model_all_data_via50/',sprintf('CV_splits_%sfeat',feature_num))
