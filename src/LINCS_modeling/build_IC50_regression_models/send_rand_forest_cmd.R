...
for (feature_num in c(200,500,1000,1500)) {
	for (hyper_num in 1:1) {
		for (fold_num in 1:10) {
			...
			command = sprintf('sbatch --wrap "./build_random_forest_models_no_tune.R --feature_num %d --hyper_slice %d --fold_number %d"', feature_num, hyper_num, fold_num)	
...
for (feature_num in c(200,500,1000,1500)) {
	for (hyper_num in 1:1) {
		for (fold_num in 1:10) {
			...
			command = sprintf('sbatch --wrap "./build_xgboost_models_no_tune.R --feature_num %d --hyper_slice %d --fold_number %d"', feature_num, hyper_num, fold_num)	
