---
title: "Make Activation/Expression Feature Selection Figures"
...
PRISM_klaeger_samples = PRISM_klaeger_imputed %>%
	left_join(klaeger_wide %>% select(drug,concentration_M,act_CDK7,act_MAPK1,act_PRKAR2A), by = c('drug'='drug', 'klaeger_conc' = 'concentration_M')) %>%
...
ggplot(PRISM_klaeger_samples, aes(x=activation,y=imputed_viability)) +
	...
	labs(x="Kinase Activation State",y="Imputed Viability") +
...
activation_hex = ggplot(PRISM_klaeger_samples, aes(x=activation,y=imputed_viability)) +
	...
	labs(x="Kinase Activation State",y="Imputed Viability",fill="Log10\nData Points") +
	BerginskiRMisc::theme_berginski() +
...
```{r}
PRISM_klaeger_samples = PRISM_klaeger_imputed %>%
	left_join(CCLE_data %>% select(DepMap_ID,exp_MED29,exp_HERC2,exp_CETN1), by=c('depmap_id' = 'DepMap_ID')) %>%
	pivot_longer(contains("exp_"), names_to = "exp_gene",values_to = "expression") %>%
	separate(exp_gene, into = c("prefix","gene"),sep="_") %>%
	mutate(gene = fct_relevel(gene, c("MED29","HERC2","CETN1")))
...
```{r}
feature_cor_sets = read_rds(here('results/single_model_exp_only_kin_regression/CV_set_correlations.rds'))
