...
klaeger_wide = read_rds(here('results/klaeger_only_kinases_tidy.rds')) %>%
...
if (file.exists(here('results/single_model_exp_only_kin_regression/CV_split_row_nums.rds'))) {
	...
	write_rds(fold_ids, here('results/single_model_exp_only_kin_regression/CV_split_row_nums.rds'))
}
...
# if (! file.exists(here('results/single_model_exp_only_kin_regression/full_model_data_set_500feat.rds'))) {
# 	all_cor = find_feature_correlations()
# 	write_rds(all_cor,here('results/single_model_exp_only_kin_regression/full_data_cor.rds'))
# 	build_regression_viability_set(all_cor,500) %>%
# 		write_rds(here('results/single_model_exp_only_kin_regression/full_model_data_set_500feat.rds'), compress='gz')
# }
...
dir.create(here('results/single_model_exp_only_kin_regression/CV_feature_cors/'), recursive = T, showWarnings = F)

write_rds(feature_cor,
					here('results/single_model_exp_only_kin_regression/CV_feature_cors/',sprintf('%04d.rds',args$CV_fold_ID)),
					compress = 'gz')
toc()