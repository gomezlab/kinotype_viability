...
for (feat_count in c(100,200,300,400,500,1000,1500,2000)) {
	model_files = Sys.glob(here('results/single_model_dose_regression/rand_forest/',
															sprintf('%dfeat_notune_pred',feat_count),
															"*"))
...
for (feat_count in c(100,200,300,400,500,1000,1500,2000)) {
	model_files = Sys.glob(here('results/single_model_dose_compound_regression/rand_forest/',
															sprintf('%dfeat_notune_pred',feat_count),
															"*"))
...
model_summary = model_results %>% 
	...
	mutate(model_type_FN = case_when(
		model_type == "rand_forest" ~ "No DC",
		model_type == "rand_forest_dose" ~ "Dose",
		model_type == "rand_forest_dc" ~ "DC"
	)) %>%
...
ggsave(here('figures/single_model_expression/model_comparison_dose_compound.png'),
			 r2_plot + rmse_plot +
...
BerginskiRMisc::trimImage(here('figures/single_model_expression/model_comparison_dose_compound.png'))
```