...
dir.create(here('results/exp_adj/exp_scale_adj'), recursive = T)
...
klaeger_wide = read_rds(here('results/exp_adj/klaeger_wide_exp_scale.rds'))
...
if (file.exists(here('results/exp_adj/CV_split_row_nums.rds'))) {
	...
	write_rds(fold_ids, here('results/exp_adj/CV_split_row_nums.rds'))
}
...
dir.create(here('results/exp_adj/exp_scale_adj/CV_feature_cors/'), recursive = T, showWarnings = F)

write_rds(feature_cor,
					here('results/exp_adj/exp_scale_adj/CV_feature_cors/',sprintf('%04d.rds',args$CV_fold_ID)),
					compress = 'gz')
toc()