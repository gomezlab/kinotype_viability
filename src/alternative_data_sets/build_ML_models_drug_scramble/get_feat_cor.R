...
dir.create(here('results/single_model_drug_scramble'), recursive = T, showWarnings = F)

source(here('src/alternative_data_sets/build_ML_models_drug_scramble/shared_feature_selection_functions.r'))
...
if (file.exists(here('results/single_model_drug_scramble/sample_scamble.rds'))) {
	sample_scramble = read_rds(here('results/single_model_drug_scramble/sample_scamble.rds'))
} else {
	...
	write_rds(sample_scramble, 
						here('results/single_model_drug_scramble/sample_scamble.rds'),
						compress = 'gz')
}
...
PRISM_klaeger_imputed = PRISM_klaeger_imputed %>%
	left_join(sample_scramble %>%
						...
						by = c('drug'='drug_start')) %>%
	select(-drug) %>%
	rename(drug = drug_scramble)

###########################################################
# Cross Validation Fold ID
###########################################################

if (file.exists(here('results/single_model_drug_scramble/CV_split_row_nums.rds'))) {
	fold_ids = read_rds(here('results/single_model_drug_scramble/CV_split_row_nums.rds'))
} else {
	...
	write_rds(fold_ids, here('results/single_model_drug_scramble/CV_split_row_nums.rds'))
}
...
dir.create(here('results/single_model_drug_scramble/CV_feature_cors/'), recursive = T, showWarnings = F)

write_rds(feature_cor,
					here('results/single_model_drug_scramble/CV_feature_cors/',sprintf('%04d.rds',args$CV_fold_ID)),
					compress = 'gz')
toc()