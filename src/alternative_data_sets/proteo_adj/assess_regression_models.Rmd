...
# Rand Forest Model Assessment - Below 5th Percentile Adj

```{r}
tic()

for (feat_count in c(100,200,300,400,500,1000,1500,2000)) {
	model_files = Sys.glob(here('results/proteo_adj/below_05_adj/rand_forest/',
															sprintf('%dfeat_notune_pred',feat_count),
															"*"))
	...
	for (this_file in model_files) {
		...
		model_results = bind_rows(
			...
			data.frame(feat_count = feat_count,
								 trees = 500,
								 fold_file = basename(this_file),
								 model_type = 'rand_forest_below_05',
								 rmse = sqrt(mean((these_cv_results$.pred - these_cv_results$target_viability)^2)),
								 r2 = glance(summary(lm(these_cv_results$.pred ~ these_cv_results$target_viability)))$r.squared))
	}
...
for (feat_count in c(100,200,300,400,500,1000,1500,2000)) {
	model_files = Sys.glob(here('results/proteo_adj/below_10_adj/rand_forest/',
															sprintf('%dfeat_notune_pred',feat_count),
															"*"))
...
for (feat_count in c(100,200,300,400,500,1000,1500,2000)) {
	model_files = Sys.glob(here('results/proteo_adj/below_20_adj/rand_forest/',
															sprintf('%dfeat_notune_pred',feat_count),
															"*"))
...
# Rand Forest Model Assessment - Below 50th Percentile Adj

```{r}
tic()

for (feat_count in c(100,200,300,400,500,1000,1500,2000)) {
	model_files = Sys.glob(here('results/proteo_adj/below_50_adj/rand_forest/',
															sprintf('%dfeat_notune_pred',feat_count),
															"*"))
...
# Rand Forest Model Assessment - Full Percentile Adj

```{r}
tic()

for (feat_count in c(100,200,300,400,500,1000,1500,2000)) {
	model_files = Sys.glob(here('results/proteo_adj/perc_rank_adj/rand_forest/',
															sprintf('%dfeat_notune_pred',feat_count),
															"*"))
	...
	for (this_file in model_files) {
		...
		model_results = bind_rows(
			...
			data.frame(feat_count = feat_count,
								 trees = 500,
								 fold_file = basename(this_file),
								 model_type = 'rand_forest_full_adj',
								 rmse = sqrt(mean((these_cv_results$.pred - these_cv_results$target_viability)^2)),
								 r2 = glance(summary(lm(these_cv_results$.pred ~ these_cv_results$target_viability)))$r.squared))
	}
...
ggsave(here('figures/single_model_expression/model_comparison_proteo_adj.png'),
			 r2_plot + rmse_plot +
...
BerginskiRMisc::trimImage(here('figures/single_model_expression/model_comparison_proteo_adj.png'))
```
