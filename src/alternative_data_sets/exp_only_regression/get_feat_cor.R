...
dir.create(here('results/single_model_exp_only_regression'), recursive = T)

source(here('src/alternative_data_sets/exp_only_regression/shared_feature_selection_functions.r'))
...
if (file.exists(here('results/single_model_exp_only_regression/CV_split_row_nums.rds'))) {
	fold_ids = read_rds(here('results/single_model_exp_only_regression/CV_split_row_nums.rds'))
} else {
	...
	write_rds(fold_ids, here('results/single_model_exp_only_regression/CV_split_row_nums.rds'))
}
...
dir.create(here('results/single_model_exp_only_regression/CV_feature_cors/'), recursive = T, showWarnings = F)

write_rds(feature_cor,
					here('results/single_model_exp_only_regression/CV_feature_cors/',sprintf('%04d.rds',args$CV_fold_ID)),
					compress = 'gz')
toc()