...
for (feature_num in c(50,100,150,200,250,300,350,400,450)) {
	for (hyper_num in 1:dim(rand_forest_grid)[1]) {
		for (fold_num in 1:10) {
			job_name = sprintf('RF_%d_%d_%d',feature_num,hyper_num,fold_num)

			command = sprintf('sbatch --job-name=%s --mem=32G --time=24:00:00 --wrap "./build_random_forest_models_no_tune.R --feature_num %d --hyper_slice %d --fold_number %d"', job_name, feature_num, hyper_num, fold_num)
