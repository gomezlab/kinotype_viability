---
title: "Make Model Predictions for JAVELIN"
...
JAVELIN_data_for_model = JAVELIN_data_long %>%
	...
	left_join(klaeger_wide, by = c('treatment' = 'drug')) %>% 
...
write_csv(JAVELIN_data_for_model, here('results/JAVELIN_data_for_model.csv'))
...
JAVELIN_data_for_model = read_csv(here('results/JAVELIN_data_for_model.csv'))

JAVELIN_data_for_model_predictions = JAVELIN_data_for_model %>% 
	select(patient, PFS_P, treatment, concentration_M, any_of(names(data_for_model_production)))
	
write_csv(JAVELIN_data_for_model_predictions, here('results/JAVELIN_data_for_model_predictions.csv'))
...
for (this_conc in unique(JAVELIN_data_for_model_predictions$concentration_M)) {
	...
	this_subset$pred_via  = predict(rand_forest_model,
																	this_subset %>%
																		mutate(depmap_id = NA, drug = NA, klaeger_conc = NA, imputed_viability = NA)) %>%
		pull(.pred)

	model_predictions = bind_rows(
		model_predictions,
		this_subset %>%
			select(patient,treatment,concentration_M, PFS_P, pred_via)
	)
}

model_predictions %>%
	arrange(patient,treatment, PFS_P,concentration_M) %>%
	write_rds(here('results/clinical_model_predictions_1500feat.rds'))
toc()
```
