...
# Rand Forest Model Assessment - Baseline
	...
	for (this_file in model_files) {
		...
		model_results = bind_rows(
			...
			data.frame(feat_count = feat_count,
								 trees = 500,
								 fold_file = basename(this_file),
								 model_type = 'rand_forest_kinase_only',
								 rmse = sqrt(mean((these_cv_results$.pred - these_cv_results$target_viability)^2)),
								 r2 = glance(summary(lm(these_cv_results$.pred ~ these_cv_results$target_viability)))$r.squared))
	}
...
model_summary = model_results %>% 
	...
	mutate(model_type_FN = case_when(
		...
		model_type == "rand_forest_kinase_only" ~ "Kinase Activation",
...
ggsave(here('figures/single_model_expression/model_comparison_only_kin_vs_all.png'),
			 r2_plot + rmse_plot +
...
BerginskiRMisc::trimImage(here('figures/single_model_expression/model_comparison_only_kin_vs_all.png'))
```
