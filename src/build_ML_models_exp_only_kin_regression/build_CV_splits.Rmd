...
```{r}
dir.create(here('results/single_model_exp_only_kin_reg'), recursive = T)
...
```{r}
if (file.exists(here('results/single_model_exp_only_kin_reg/CV_split_row_nums.rds'))) {
	fold_ids = read_rds(here('results/single_model_exp_only_kin_reg/CV_split_row_nums.rds'))
} else {
	fold_ids = sample(rep(1:10,length.out = dim(PRISM_klaeger_imputed)[1]))
	write_rds(fold_ids, here('results/single_model_exp_only_kin_reg/CV_split_row_nums.rds'))
}
```

```{r}
if (! file.exists(here('results/single_model_exp_only_kin_reg/full_model_data_set_1500feat.rds'))) {
	all_cor = find_feature_correlations()
	build_regression_viability_set(all_cor,1500) %>%
		write_rds(here('results/single_model_exp_only_kin_reg/full_model_data_set_1500feat.rds'), compress='gz')
}
```

```{r}
tic()
if (file.exists(here('results/single_model_exp_only_kin_reg/CV_set_correlations.rds'))) {
	feature_cor_sets = read_rds(here('results/single_model_exp_only_kin_reg/CV_set_correlations.rds'))
} else {
	...
	write_rds(feature_cor_sets, 
						here('results/single_model_exp_only_kin_reg/CV_set_correlations.rds'), 
						compress = 'gz')
}
toc()
```
...
for (i in 1:10) {
	...
	for (feature_num in c(50,100,150,200,300,400,500,1000,1500,2000,3000,4000,5000)) {
		target_dir = here('results/single_model_exp_only_kin_reg/',sprintf('CV_splits_%sfeat',feature_num))
