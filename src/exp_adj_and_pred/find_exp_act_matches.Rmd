...
read_rds(here('results/klaeger_full_tidy.rds')) %>%
	left_join(CCLE_data_act_summary %>% 
							filter(DepMap_ID %in% PRISM_lines) %>%
							select(-log2_TPM), by=c('gene_name'='act_gene')) %>%
	
	mutate(relative_intensity = ifelse(zero_exp, 1, relative_intensity)) %>%
	
	mutate(act_gene_name = paste0("act_",gene_name)) %>%
	select(-gene_name, -contains("below_"), -zero_exp) %>%
	pivot_wider(names_from = act_gene_name, values_from = relative_intensity) %>%
...
read_rds(here('results/klaeger_full_tidy.rds')) %>%
	left_join(CCLE_data_act_summary %>%
							ungroup() %>%
							filter(DepMap_ID %in% PRISM_lines) %>%
							select(act_gene,below_10_per,DepMap_ID), by=c('gene_name'='act_gene')) %>%

	mutate(relative_intensity = ifelse(below_10_per, 1, relative_intensity)) %>%
	mutate(act_gene_name = paste0("act_",gene_name)) %>%
	select(-gene_name, -below_10_per) %>%
	pivot_wider(names_from = act_gene_name, values_from = relative_intensity) %>%
	write_rds(here('results/exp_adj/klaeger_wide_below_10_per_gene.rds'), compress = 'gz')
