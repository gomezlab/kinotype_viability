...
parser$add_argument('--trees', default = 1000, type="integer")

args = parser$parse_args()
print(sprintf('Fold: %02d',args$CV_fold_ID))

dir.create(here('results/single_model_expression_regression/', 
								sprintf('rand_forest_tune/%dfeat_%dtrees/',args$feature_num,args$trees)), 
...
dir.create(here('results/single_model_expression_regression/', 
								sprintf('rand_forest_tune/%dfeat_%dtrees_pred/',args$feature_num,args$trees)), showWarnings = F)

pred_output_file = here('results/single_model_expression_regression/', 
												sprintf('rand_forest_tune/%dfeat_%dtrees_pred/',args$feature_num,args$trees),
												sprintf('fold%04d_test.rds',args$CV_fold_ID))
