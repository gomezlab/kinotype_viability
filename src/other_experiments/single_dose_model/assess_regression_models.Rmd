...
for (feat_count in c(100,200,300,400,500,600,700,800,900,1000,1500,2000,3000,4000,5000)) {
	model_files = Sys.glob(here('results/single_dose_model/rand_forest/',
...
for (feat_count in c(100,200,300,400,500,600,700,800,900,1000,1500,2000,3000,4000,5000)) {
	model_files = Sys.glob(here('results/single_dose_model/rand_forest_no_klaeger/',
															sprintf('%dfeat_notune_pred',feat_count),
															"*"))
	...
	for (this_file in model_files) {
		...
		model_results = bind_rows(
			...
			data.frame(feat_count = feat_count,
								 trees = 500,
								 fold_file = basename(this_file),
								 model_type = 'rand_forest_no_klaeger',
								 rmse = sqrt(mean((these_cv_results$.pred - these_cv_results$target_viability)^2)),
								 r2 = glance(summary(lm(these_cv_results$.pred ~ these_cv_results$target_viability)))$r.squared))
	}
}
...
model_summary = model_results %>% 
	...
	mutate(model_type_FN = case_when(
		...
		model_type == "rand_forest_no_klaeger" ~ "Expression Only",
		model_type == "rand_forest_no_exp" ~ "Random Forest-Nexp",
	)) %>%
	select(model_type,feat_count,trees,r2_estimate,rmse_estimate,everything()) %>%
	mutate(model_type_FN = as.factor(model_type_FN)) %>%
	filter(feat_count <= 2000)
...
```{r}
dir.create(here('figures/single_dose_model'), recursive = T)
...
ggsave(here('figures/single_dose_model/model_comparison.png'),
			 r2_plot + rmse_plot +
...
BerginskiRMisc::trimImage(here('figures/single_dose_model/model_comparison.png'))
```