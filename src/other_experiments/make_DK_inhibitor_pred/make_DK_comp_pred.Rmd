...
DK_drug_set = bind_rows(
	fill_in_activation(CK2_inhib_pseudo_k) %>% mutate(drug = "SGC-CK2-1 - CSNK2A2"),
	fill_in_activation(PIP4K2C_inhib_pseudo_k) %>% mutate(drug = "BI00048423 - PIP4K2C"),
	fill_in_activation(CAMKK2_inhib_pseudo_k) %>% mutate(drug = "SGC-CAMKK2-1 - CAMKK2"),
	fill_in_activation(DYRK1B_inhib_pseudo_k) %>% mutate(drug = "BI00036838 - DYRK1B"),
	fill_in_activation(neg_ctrl) %>% mutate(drug = "Neg Ctrl"),
)
...
model_predictions$viability = predict(rand_forest_model,data_for_model %>%
																				mutate(klaeger_conc = NA, imputed_viability = NA, depmap_id = NA)) %>%
	pull(.pred)

# write_rds(rand_forest_model$fit$fit, here('results/single_model_expression_regression/only_model_fit.rds'), compress = 'gz')
# 
# rand_forest_model = read_rds(here('results/single_model_expression_regression/only_model_fit.rds'))
# 
# tic(); model_predictions = data_for_model %>% select(DepMap_ID,drug,concentration_M); model_predictions$viability = predict(rand_forest_model,data_for_model %>% mutate(klaeger_conc = NA, imputed_viability = NA, depmap_id = NA)) %>% pull(.pred); toc();

model_pred_summary = model_predictions %>%
	group_by(concentration_M, drug) %>%
	summarise(mean_via = mean(viability),
						conf.low = tidy(t.test(viability))$conf.low,
						conf.high = tidy(t.test(viability))$conf.high,
						quat_025 = quantile(viability,0.025),
						quat_975 = quantile(viability,0.975))
```
...
model_pred_TNBC = model_predictions %>%
	...
	# 						rename(viability = mean_via)) %>%
	identity()
```

```{r}

ggplot(model_pred_TNBC, aes(x=log10(concentration_M), y=viability)) +
