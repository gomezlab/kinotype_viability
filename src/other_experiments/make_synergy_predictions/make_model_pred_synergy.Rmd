...
NCI_almanac_lines = read_csv(here('results/matched_ALMANAC_cell_lines.csv')) %>%
...
if (file.exists(here('results/synergy_pred/klaeger_synergy_combos.rds'))) {
	klaeger_combo_sets = read_rds(here('results/synergy_pred/klaeger_synergy_combos.rds.gz'))
} else {
	...
	klaeger_combo_sets = possible_klaeger_combos %>% 
		...
		write_rds(here('results/synergy_pred/klaeger_synergy_combos.rds.gz'), compress = 'gz')
}
toc()
```
...
for (this_line in unique(CCLE_data_filtered$DepMap_ID)) {
	all_pred = c()
