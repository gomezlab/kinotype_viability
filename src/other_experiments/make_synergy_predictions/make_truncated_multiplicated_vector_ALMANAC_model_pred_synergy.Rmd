...
rand_forest_model = read_rds(here('results/final_model_500feat.rds'))
...
```{r}
ALMANAC_klaeger_matches = read_csv(here('results/klaeger_ALMANAC_matches.csv'))
klaeger_wide_NCI_matched = read_rds(here('results/klaeger_full_tidy.rds')) %>%
	mutate(act_gene_name = paste0("act_",gene_name)) %>%
	select(-gene_name) %>% 
	filter(drug %in% ALMANAC_klaeger_matches$klaeger_name) %>% 
	pivot_wider(names_from = act_gene_name, values_from = relative_intensity)
