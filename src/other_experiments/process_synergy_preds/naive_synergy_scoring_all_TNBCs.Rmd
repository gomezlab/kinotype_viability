...
this_single_preds = All_preds %>% 
	filter(depmap_id == query_depmap_id)

this_single_preds_zeros = this_single_preds %>% 
...
this_combo_summary = this_synergy_predictions_file %>%
  #join in single predictions
	inner_join(rename(this_single_preds_complete, 'single_1_pred_via' = pred_via), 
					 by = c('drug_1' = 'drug' ,
					        'concentration_M_d1' = 'concentration_M', 
					        'depmap_id')) %>% 
	inner_join(rename(this_single_preds_complete, 'single_2_pred_via' = pred_via), 
					 by = c('drug_2' = 'drug' ,
...
```{r}
All_TNBC_synergy_scores= data.frame()
for(i in 1:dim(TNBC_data_table)[1]) {
	this_TNBC_scores = get_naive_synergy_scores(query_depmap_id = TNBC_data_table$DepMap_ID[i], All_synergy_data = All_TNBC_synergy_data)
		All_TNBC_synergy_scores = bind_rows(All_TNBC_synergy_scores, this_TNBC_scores)
}

combo_indexes = All_TNBC_synergy_scores %>% 
	select(depmap_id, drug_1, drug_2) %>% 
	unique() %>% 
	mutate(combo_index = 1:n())

All_TNBC_synergy_scores_summary = All_TNBC_synergy_scores %>% 
...
write_csv(All_TNBC_synergy_scores_summary, here('results/All_complete_TNBC_lines_synergy_scores.csv'))
```
...
All_TNBC_synergy_scores_summary = read_csv(here('results/All_complete_TNBC_lines_synergy_scores.csv'))
...
for (i in 1:dim(this_synergy_scores)[1]) {
	dir.create(here('figures/synergy_preds', paste0(this_synergy_scores$stripped_cell_line_name[i])), showWarnings = F)
	get_synergy_heatmaps(combo_index = this_synergy_scores$combo_index[i], 
											 synergy_summary_dataset = All_TNBC_synergy_scores_summary)
	ggsave(here('figures/synergy_preds/',paste0(this_synergy_scores$stripped_cell_line_name[i],"/"), paste0('top_',i,'_',this_synergy_scores$stripped_cell_line_name[i],'_synergy.png')), height = 5, width = 15)
}
}
...
get_top_4_synergy_heatmaps(depmap_id_to_search = avail_lines$DepMap_ID[i], 
													 synergy_scores_dataset = All_TNBC_synergy_scores_summary)
}
```
