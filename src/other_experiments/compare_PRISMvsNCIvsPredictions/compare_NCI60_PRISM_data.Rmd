...
response_curve_parameters = PRISM %>% 
 ...
 #convert log molar to molar 
	...
	mutate(
		imputed_viability = lower_limit + (upper_limit - lower_limit)/(1 + exp(slope * (log(NCI_conc*10^6) - log(ec50))))) %>%
	...
	write_rds(here('results/PRISM_imputed_for_NCI60_tidy.rds'), compress = 'gz')
```

```{r}
PRISM_imputed_for_NCI_60 = read_rds(here('results/PRISM_imputed_for_NCI60_tidy.rds'))
...
NCI_PRISM_combined_viabilities %>% 
	ggplot(aes(imputed_viability, AVERAGE_PTC)) +
	geom_hex() +
	scale_fill_gradient(low="lightblue1",high="darkblue",trans="log10") +
	geom_smooth() +
	labs(title = paste0('R\u00B2 = ', 
											round(
												(cor(NCI_PRISM_combined_viabilities$imputed_viability, 
														NCI_PRISM_combined_viabilities$AVERAGE_PTC)^2),
												2),
											' / RMSE = ',
											round(
												rmse(
													NCI_PRISM_combined_viabilities$AVERAGE_PTC,
													NCI_PRISM_combined_viabilities$imputed_viability),
												2)),
			 x = "PRISM Viability",
			 y = "NCI-60 Viability") + 
	geom_abline(intercept = 0, slope = 1, size = 0.5, colour = 'red') 
...
ggsave(here('figures/NCI60_vs_PRISM_viability_correlation.png'), height = 7, width = 10)	
