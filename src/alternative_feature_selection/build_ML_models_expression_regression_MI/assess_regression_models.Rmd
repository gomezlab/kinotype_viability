...
for (feat_count in c(100,200,300,400,500,600,700,800,900,1000,1500,2000)) {
	model_files = Sys.glob(here('results/single_model_expression_regression_MI/rand_forest/',
															sprintf('%dfeat_notune_pred',feat_count),
															"*"))
...
for (feat_count in c(100,200,300,400,500,600,700,800,900,1000,1500,2000)) {
	model_files = Sys.glob(here('results/single_model_expression_regression_cluster/rand_forest/',
															sprintf('%dfeat_notune_pred',feat_count),
															"*"))
...
model_summary = model_results %>% 
	group_by(feat_count,trees,model_type,selection) %>% 
	nest() %>%
...
r2_plot = ggplot(model_summary, 
								 aes(y=r2_estimate,x=feat_count, color=selection, shape=selection)) + 
...
rmse_plot = ggplot(model_summary, 
									 aes(y=rmse_estimate,x=feat_count, color=selection, shape=selection)) + 
...
ggsave(here('figures/cor_vs_mutual_info/model_comparison_MI_vs_Cor.png'),
			 r2_plot + rmse_plot +
...
BerginskiRMisc::trimImage(here('figures/cor_vs_mutual_info/model_comparison_MI_vs_Cor.png'))
```
