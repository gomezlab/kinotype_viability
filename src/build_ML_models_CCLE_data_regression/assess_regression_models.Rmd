...
for (hyper in 1:10) {
	model_files = Sys.glob(here('results/single_model_regression/rand_forest_param_scan/',sprintf('hyper%03d*.rds', hyper)))
	...
	model_results = bind_rows(
		model_results,
		data.frame(hyper_set = hyper,
							 rmse = sqrt(mean((hyper_set_predictions$.pred - hyper_set_predictions$target_imputed_viabiity)^2)),
							 r2 = glance(summary(lm(hyper_set_predictions$.pred ~ hyper_set_predictions$target_imputed_viabiity)))$r.squared)
	)
	
	print(paste0("Done with ",hyper))
}
...
model_files = Sys.glob(here('results/single_model_via_50/rand_forest_param_scan/',sprintf('hyper%03d*.rds', best_model_results$hyper_set)))
...
dir.create(here('figures/single_model_via_50/'), recursive = T, showWarnings = F)
ggsave(here('figures/single_model_via_50/roc_prc.png'), width=5,height=2.5)
BerginskiRMisc::trimImage(here('figures/single_model_via_50/roc_prc.png'))
```
