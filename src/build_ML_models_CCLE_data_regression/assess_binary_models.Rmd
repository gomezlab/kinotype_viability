...
```{r}
tic()
if (file.exists(here('results/single_model_via_50/hyper_param_results.csv'))) {
	model_roc_prc = read_csv(here('results/single_model_via_50/hyper_param_results.csv'))
} else {
	model_roc_prc = data.frame()
	
	for (hyper in 1:10) {
		model_files = Sys.glob(here('results/single_model_via_50/rand_forest_param_scan/',sprintf('hyper%03d*.rds', hyper)))
	...
	write_csv(model_roc_prc,here('results/single_model_via_50/hyper_param_results.csv'))
}
toc()
```
