...
for (hyper_num in seq(1,10)) {
	for (fold_num in 1:10) {
		...
		if (file.exists(here('results/single_model_plus_kinases/rand_forest_param_scan/',target_file))) {
			next
		} else {
			missing = missing + 1
			...
			local_command = sprintf('./build_random_forest_models.R --hyper_slice %d --fold_number %d', hyper_num,fold_num)
