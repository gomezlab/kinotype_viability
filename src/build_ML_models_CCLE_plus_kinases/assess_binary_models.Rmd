...
# Rand Forest Model Assessment + Kinases

```{r}
tic()

model_roc_prc_kin = data.frame()

for (hyper in 1:10) {
	model_files = Sys.glob(here('results/single_model_plus_kinases/rand_forest_param_scan/',sprintf('hyper%03d*.rds', hyper)))
...
model_roc_prc_kin = model_roc_prc_kin %>% 
	mutate(prc_rank = min_rank(desc(prc)), 
				 roc_rank = min_rank(desc(roc)))
